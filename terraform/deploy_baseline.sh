#!/bin/bash
# example command
# ./deploy_baseline.sh eu-west-1 60363d82783fd71b1e508cd5 iruhenwv 4b01113b-c9f1-430d-89c9-4b6cbfa1ed4d demon mustafari221  base apply
region=$1
orgId=$2
orgPubKey=$3
orgPrivKey=$4
...
terraform init -reconfigure \
    ...
    -backend-config="key=tf-bootstrap-${projectName}-${environment}"

terraform $mode -var="org_api_pri_key=${orgPrivKey}" -var="org_api_pub_key=${orgPubKey}" -var="org_id=${orgId}" -var="environment_name=${environment}" -var="project_name=${projectName}" -var="region=${region}" -auto-approve
